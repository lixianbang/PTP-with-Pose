...
def draw_line(img, traj, color, ratio, thickness=2, skip=1):
    for idx, (d1, d2) in enumerate(zip(traj[:-1], traj[1:])):
        if idx % skip != 0:
            continue
        cv2.line(img, (int(d1[0]*ratio), int(d1[1]*ratio-200)), (int(d2[0]*ratio), int(d2[1]*ratio-200)), color, thickness)
    return img
...
def draw_dotted_line(img, traj, color, ratio, thickness=2, r=0.85):
    for idx, (d1, d2) in enumerate(zip(traj[:-1], traj[1:])):
        if r < 0.5:
            ...
            cv2.line(img, (x1, y1-200), (x2, y2-200), color, thickness)
...
def draw_x(img, point, color, ratio, thickness=2):
    ...
    scale = thickness * 2
    cv2.line(img, (x - scale, y - scale-200), (x + scale, y + scale-200), color, thickness)
    cv2.line(img, (x + scale, y - scale-200), (x - scale, y + scale-200), color, thickness)
    return img


def draw_rect(img, point, color, ratio, rsize=12):
    x, y = int(point[0]*ratio), int(point[1]*ratio-200)
    cv2.rectangle(img, (x - rsize // 2, y - rsize // 2), (x + rsize // 2, y + rsize // 2), color, -1)
    return img


def draw_scale(img, cx, cy, rad, color):
    cv2.circle(img, (cx, cy-200), rad, color, 1)
    cv2.line(img, (cx, cy - rad-200), (cx, cy + rad-200), color, 1)
    cv2.line(img, (cx - rad, cy-200), (cx + rad, cy-200), color, 1)
    return img
