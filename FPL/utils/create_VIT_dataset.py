...
if __name__ == "__main__":
    ...
    parser.add_argument('--indir_list', type=str, default='id_vit.txt')
    ...
    out_fn = "dataset/vit_test.joblib"
    ...
    for video_id, nb_images in zip(vid_list, nb_image_list):
        trajectory_path = os.path.join(
            data_dir, "out2FPL.json".format(video_id))
        with open(trajectory_path, "r") as f:
            trajectory_dict = json.load(f)

        total_frames += nb_images
        if args.ego_type == "sfm":
            egomotion_path = os.path.join(data_dir, "egomotions/GOPR0234U20_egomotion.csv")
