...
class SummaryLogger(object):
  ...
  def write(self):
    ...
    new_summary = pd.DataFrame([self._summ.values()], index=[
                               self._date_str], columns=self._summ.keys())
    try:
    ...
    if self._date_str in current.index:
      ...
      current = current.reindex(current.columns.union(new_summary.columns), axis=1)
